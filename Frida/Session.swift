...
public class Session: NSObject, NSCopying {
    ...
    public var pid: UInt {
        return UInt(telco_session_get_pid(handle))
    }

    public var persistTimeout: UInt {
        return UInt(telco_session_get_persist_timeout(handle))
    }

    public var isDetached: Bool {
        return telco_session_is_detached(handle) != 0
    }

    public override var description: String {
        return "Telco.Session(pid: \(pid))"
    }
    ...
    public func detach(_ completionHandler: @escaping DetachComplete = {}) {
        Runtime.scheduleOnTelcoThread {
            telco_session_detach(self.handle, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<DetachComplete>>.fromOpaque(data!).takeRetainedValue()

                telco_session_detach_finish(OpaquePointer(source), result, nil)
    ...
    public func resume(_ completionHandler: @escaping ResumeComplete = { _ in }) {
        Runtime.scheduleOnTelcoThread {
            telco_session_resume(self.handle, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<ResumeComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                telco_session_resume_finish(OpaquePointer(source), result, &rawError)
    ...
    public func enableChildGating(_ completionHandler: @escaping EnableChildGatingComplete = { _ in }) {
        Runtime.scheduleOnTelcoThread {
            telco_session_enable_child_gating(self.handle, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<EnableChildGatingComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                telco_session_enable_child_gating_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func disableChildGating(_ completionHandler: @escaping DisableChildGatingComplete = { _ in }) {
        Runtime.scheduleOnTelcoThread {
            telco_session_disable_child_gating(self.handle, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<DisableChildGatingComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                telco_session_disable_child_gating_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func createScript(_ source: String, name: String? = nil, runtime: ScriptRuntime? = nil, completionHandler: @escaping CreateScriptComplete) {
        Runtime.scheduleOnTelcoThread {
            ...
            telco_session_create_script(self.handle, source, options, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<CreateScriptComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawScript = telco_session_create_script_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func createScript(_ bytes: Data, name: String? = nil, runtime: ScriptRuntime? = nil, completionHandler: @escaping CreateScriptComplete) {
        Runtime.scheduleOnTelcoThread {
            ...
            telco_session_create_script_from_bytes(self.handle, rawBytes, options, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<CreateScriptComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawScript = telco_session_create_script_from_bytes_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func compileScript(_ source: String, name: String? = nil, runtime: ScriptRuntime? = nil, completionHandler: @escaping CompileScriptComplete) {
        Runtime.scheduleOnTelcoThread {
            ...
            telco_session_compile_script(self.handle, source, options, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<CompileScriptComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawBytes = telco_session_compile_script_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    private static func parseScriptOptions(_ name: String?, _ runtime: ScriptRuntime?) -> OpaquePointer {
        let options = telco_script_options_new()!
        ...
        if let runtime = runtime {
            telco_script_options_set_runtime(options, TelcoScriptRuntime(runtime.rawValue))
        }
    ...
    public func setupPeerConnection(stunServer: String? = nil, relays: [Relay]? = nil,
        ...
        Runtime.scheduleOnTelcoThread {
            let options = telco_peer_options_new()
            defer {
                g_object_unref(gpointer(options))
            }

            if let stunServer = stunServer {
                telco_peer_options_set_stun_server(options, stunServer)
            }

            for relay in relays ?? [] {
                telco_peer_options_add_relay(options, relay.handle)
            }

            telco_session_setup_peer_connection(self.handle, options, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<SetupPeerConnectionComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                telco_session_setup_peer_connection_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
    ...
    public func joinPortal(_ address: String, certificate: String? = nil, token: String? = nil, acl: [String]? = nil,
        ...
        Runtime.scheduleOnTelcoThread {
            let options = telco_portal_options_new()
            defer {
                g_object_unref(gpointer(options))
            }
            ...
            if let certificate = certificate {
                do {
                    ...
                    telco_portal_options_set_certificate(options, rawCertificate)
            ...
            if let rawAcl = rawAcl {
                telco_portal_options_set_acl(options, rawAcl, aclLength)
                g_strfreev(rawAcl)
            }

            telco_session_join_portal(self.handle, address, options, nil, { source, result, data in
                let operation = Unmanaged<AsyncOperation<JoinPortalComplete>>.fromOpaque(data!).takeRetainedValue()

                var rawError: UnsafeMutablePointer<GError>? = nil
                let rawMembership = telco_session_join_portal_finish(OpaquePointer(source), result, &rawError)
                if let rawError = rawError {
                    let error = Marshal.takeNativeError(rawError)
                    Runtime.scheduleOnMainThread {
                        operation.completionHandler { throw error }
